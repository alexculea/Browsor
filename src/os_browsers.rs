...
mod winapi {
  ...
  pub use ::winapi::um::shellapi::{SHGetFileInfoW, SHFILEINFOW, SHGFI_LARGEICON, SHGFI_ICON};
...
pub fn get_exe_file_icon(path: &str) -> Result<winapi::HICON> {
  ...
  let mut file_info: winapi::SHFILEINFOW = unsafe { std::mem::MaybeUninit::zeroed().assume_init() };
  let res = unsafe { winapi::SHGetFileInfoW(
    wide_path.as_ptr(), 
    0, 
    &mut file_info, 
